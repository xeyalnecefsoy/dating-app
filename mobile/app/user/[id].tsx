...
import { ArrowLeft, CheckCircle2, Heart, MessageCircle } from "../../lib/icons";
...
export default function UserProfileScreen() {
  ...
  const userByUsername = useQuery(api.users.getUserByUsername, id ? { username: id } : "skip");
    ...
    <>
      ...
      <ScrollView style={styles.container} contentContainerStyle={styles.scrollContent}>
        ...
        {user.avatar ? (
          ...
          <View style={[styles.heroImage, { backgroundColor: Colors.surfaceDark, justifyContent: "center", alignItems: "center" }]}>
            <Text style={{ fontSize: 64, color: Colors.mutedForeground }}>👤</Text>
          </View>
        )}
            ...
            <View style={styles.chipsRow}>
              {user.values.map((v: string, i: number) => (
                <View key={i} style={[styles.chip, { backgroundColor: "rgba(59,130,246,0.15)" }]}>
                  <Text style={[styles.chipText, { color: Colors.infoBlue }]}>{v}</Text>
                </View>
              ))}
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  name: { fontSize: 28, fontWeight: "800", color: Colors.foreground },
  username: { color: Colors.primary, fontWeight: "600", fontSize: 14, marginTop: 4 },
  location: { color: Colors.mutedForeground, fontSize: 14, marginTop: 6 },
  card: {
  ...
  cardLabel: { color: Colors.mutedForeground, fontSize: 12, fontWeight: "700", textTransform: "uppercase", marginBottom: 8 },
  cardText: { color: Colors.mutedForeground, fontSize: 14, lineHeight: 22 },
  chipsRow: { flexDirection: "row", flexWrap: "wrap", gap: 8 },
  chip: { backgroundColor: Colors.primaryAlpha15, paddingHorizontal: 14, paddingVertical: 7, borderRadius: 16 },
  ...
  infoLabel: { color: Colors.mutedForeground, fontSize: 13, marginLeft: 10, flex: 1 },
  infoValue: { color: Colors.foreground, fontSize: 14, fontWeight: "600" },
});
